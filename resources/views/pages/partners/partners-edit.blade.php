            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-block">
                        <form method="POST" action="/partners/{{$partners->id}}" onsubmit="document.getElementById('submit_partner').setAttribute('disabled', 'disabled')">
                   @method('put')
                  {{-- #############################################FILED ############################## --}}
                            <div class="form-group row">
                                <label for="name" class="col-sm-3 col-form-label">Libellé</label>
                                <div class="col-sm-3">
                                    <input required  value="{{$partners->name}}" name="name" id="name" type="text"
                  ...
                  {{-- #############################################FILED ############################## --}}
                            <div class="form-group row">
                                ...
                                <div class="col-sm-3">
                                    <input required  value="{{$partners->phone}}" name="phone" id="phone" type="text"
                  ...
                  {{-- #############################################FILED ############################## --}}
                            <div class="form-group row">
                                ...
                                <div class="col-sm-3">
                                    <input required  value="{{$partners->email}}" name="email" id="email" type="email"
                                ...
                                <div class="col-sm-3">
                                    <textarea
                                        required   name="adress" id="adress"
                                           class="form-control form-control-normal" placeholder="Adresse">{{$partners->adress}}</textarea>
                                    @error('adress')
                                    <div  class="invalid-feedback ">
                                        {{ $message }}
                                    </div>
                                ...
                                <div class="col-sm-3">
                                    <select  required  name="countries_id" id="countries_id" class="">
                                        ...
                                        @foreach(countries() as $country)
                                            <option @if($partners->countries_id == $country->id  ) selected @endif value="{{$country->id}}"> {{$country->name}} </option>
                                        @endforeach
